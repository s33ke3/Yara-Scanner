...
def run(args):
    ...
    if args["debug"] != None and args["debug"] != '':
        debugLogItems = args["debug"].split("|")
        settings.debug_log_enabled = True
        settings.debug_log_level = debugLogItems
...
|_  _||_  _|                       .' ____ \                                                
  ...
  |______|\'-;__/[___]   \'-;__/    \______.''.___.'\'-;__/[___||__][___||__]'.__.'[___]    
    ...
    --> iomoath yara-scanner Modded 2.6 by CyberDivision - @  <--
    """
    ...
    ap.add_argument("-d", "--debug", action='store', type=str,
                    help="List of Debug loggin types to show separate by | (INFO|DEBUG|WARNING|ERROR|CRITICAL).")

    ap.add_argument("--version", action="version", version='Yara-Scanner Version 1.0')
    return ap

def main():
    print(socket.gethostname()) #Print Machine Hostname
    global arg_parser
